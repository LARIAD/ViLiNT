...
def main(args: argparse.Namespace):
    ...
    # Iterate recursively through args.input_dir and collect ROS1 bag files.
    bag_files = []
    ...
    for bag_path in tqdm.tqdm(bag_files, desc="Bags processed"):
        ...
        if 'lidar_process_func' in config[args.dataset_name]:
            ...
            if bag_img_data is None or bag_lidar_data is None or bag_traj_data is None:
                print(
                    f"{bag_path} did not have the topics we were looking for. Skipping..."
                )
                continue
            cut_trajs = filter_backwards_with_lidar(bag_img_data, bag_lidar_data, bag_traj_data)
            ...
            for i, (img_data_i, lidar_data_i, traj_data_i) in enumerate(cut_trajs):
                ...
                for i, img in tqdm.tqdm(enumerate(img_data_i),desc="save images"):
                    img.save(os.path.join(traj_folder_i, f"{i}.jpg"))
                for i, lidar in tqdm.tqdm(enumerate(lidar_data_i), desc="save lidar"):
                    np.save(os.path.join(traj_folder_i, f"{i}.npy"), lidar)
        else:
